(function($) {
    ...
    $.fn.setCustomScrollbars = function(options) {
        options = options || {};
        autohide = options.autohide;
        padding_y = options.padding || options.padding_y || 0;
        padding_x = options.padding || options.padding_x || 0;
        // Flags
        ...
        this.mainLoop = function() {
            for (var i = 0; elements[i] !== undefined; i++) {
                if (needScrollbars(elements[i]) && !$(elements[i]).hasClass('no-custom-scroll')) {
        ...
        this.Update = function() {
            for (var i = 0; elements[i] !== undefined; i++) {
                if (needScrollbars(elements[i]) && !$(elements[i]).hasClass('no-custom-scroll')) {
                    ...
                    // get some values before the element is wrapped
                    getDimensions(target, false, true);
        ...
        function setEvents(elem) {
            ...
            if (addVScroll) {
                el.find('.lb-v-scrollbar-slider').on('mousedown', function(e) {
                    eventY = e.pageY;
                    VDragging = true;
                    activeScroll = $(this);
                ...
                el.find('.lb-v-scrollbar').on('mousedown', function(e) {
            ...
            if (addHScroll) {
                el.find('.lb-h-scrollbar-slider').on('mousedown', function(e) {
                    eventX = e.pageX;
                    HDragging = true;
                    activeScroll = $(this);
                    activeWrap = el.find('.lb-wrap');
                ...
                el.find('.lb-h-scrollbar').on('mousedown', function(e) {
        ...
        function setSlidersHeight(elem) {
            ...
            el.find('.lb-v-scrollbar-slider').css({ "height": vSliderHeight - padding_y });
            el.find('.lb-h-scrollbar-slider').css({ "width": hSliderHeight - padding_x });
        }
        ...
        function reduceScrollbarsWidthHeight(elem) {
            ...
            if (addVScroll && addHScroll) {
                ...
                el.find('.lb-v-scrollbar').css({ "height": vLbHeight - 2 * padding_y, "margin-top": padding_y });
                el.find('.lb-h-scrollbar').css({ "width": hLbHeight - 2 * padding_x, "margin-left": padding_x });
            } else {
        ...
        function hideScrollbars(elem, vscroll, hscroll) {
            ...
            if (vscroll || hscroll) {
                el.addClass('lb-scroll-container-overflow-hidden');
                el.removeClass('lb-scroll-container-overflow-auto');
        ...
        function resizeInnerWrap(main, child) {
            var mainEl = $(main);
            var childEl = $(child);
            mainEl.addClass('lb-scroll-container-position-relative');
        ...
        function setVScrollbarWidth(elem) {
            var el = $(elem);
            el.addClass('lb-scroll-container-overflow-auto');
            el.removeClass('lb-scroll-container-overflow-hidden');
        ...
        function getDimensions(elem, scroll, update) {
            var el = $(elem).get(0);
            if (update) {
                el = $(el).find('.lb-wrap').get(0);
            }
            scrollHeight = ((typeof(scroll) != 'undefined' && scroll != false) ? scroll.height : el.scrollHeight) - padding_y;
            scrollWidth = ((typeof(scroll) != 'undefined' && scroll != false) ? scroll.width : el.scrollWidth) - padding_x;
            clientHeight = (el.clientHeight) - padding_y;
            clientWidth = (el.clientWidth) - padding_x;
            offsetHeight = (el.offsetHeight) - padding_y;
            offsetWidth = (el.offsetWidth) - padding_x;
