...
import { schemas, CustomDictError } from "../types/mod.ts";
import { IExerciseService, IExerciseStore, IJWTService, ISubjectStore, ITeamStore, IUser, IUserStore } from "../interfaces/mod.ts";
...
export class SessionController extends Authorizer {
    ...
    private unicodify(x: number | null) {
        //return (x??{toString:()=>"null"}).toString();
        switch (x) {
          case null:
            return "☐";
          case 0:
    ...
    async add(ctx: RouterContext) {
        ...
        if (!await this.isAssigneeOf(subject, user) || !await this.isAssignee(teamId, user.id)) {
            throw new httpErrors["Forbidden"]();
        } //! P
        const team = this.ts.get(teamId);
        if (!await team.exists())
    ...
    async list(ctx: RouterContext) {
        ...
        ctx.response.body = await Promise.all(
            (await team.session.exercises.get()).map(async (e) => {
                const ex = await this.ex.render(this.es.deuid(e)!, { seed: (await user.seed.get())! + await team.session.seedOffset.get() }); // teacher is not allowed
    ...
    async status(ctx: RouterContext) {
        ...
        ctx.response.body = {
            ...
            report: Object.entries(report).map(([userId, correctness]) => ({ userId, exercises: exercises.map(e => this.unicodify(correctness[e] ?? null)) }))
        } //! OD
