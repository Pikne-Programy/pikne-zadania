...
class Button {
    ...
    get viewBox() {
        return this._viewBox
            ? `0 0 ${this._viewBox[0]} ${this._viewBox[1]}`
            : '';
    }
...
class ListButton extends Button {
    constructor(
        ...
        super(label, icon, action, viewBox, isText);
    }
}

export class InputButton extends Button {
    fields: InputButtonField[];
    ...
    constructor(
        ...
        this.fields = fields.map(
            ([name, type, isRequired]) =>
                new InputButtonField(name, type, isRequired)
        );
    }
...
class InputButtonField {
    ...
    isValid(): boolean | null {
        ...
        return Utils.INPUT_TYPES_MAP[this.type].test(this.value);
    }
}
...
export class EditorToolbarComponent implements OnInit, OnDestroy {
    ...
    private addVariable(values: string[]) {
        ...
        if (values.length === 3 && sliced) {
            const variable = Utils.createVariable(
                values[0],
                values[1],
                values[2]
            );
            this.formField.setValue(sliced.pre + variable + sliced.post);
            if (sliced.selectionStart !== sliced.selectionEnd) {
                ...
                this.updateTextarea(sliced.pre.length + variable.length, null);
        }
    }
    ...
    private addRange(values: string[]) {
        ...
        if (values.length === 5 && sliced) {
            ...
            this.formField.setValue(sliced.pre + range + sliced.post);
            if (sliced.selectionStart !== sliced.selectionEnd) {
                this.updateTextarea(
                    sliced.pre.length,
    ...
    private addUnknown(values: string[]) {
        ...
        if (values.length === 2 && sliced) {
            ...
            this.formField.setValue(sliced.pre + unknown + sliced.post);
            if (sliced.selectionStart !== sliced.selectionEnd) {
                this.updateTextarea(
                    sliced.pre.length,
                ...
                this.updateTextarea(sliced.pre.length + unknown.length, null);
        }
    }

    private sliceText(): SlicedText | null {
        if (this.selection) {
            const value = this.formField.value as string;
