...
v0.5, 2022-05-27: Revision 2
:issue: https://github.com/Pikne-Programy/pikne-zadania/issues/50[#50]
:stylesheet: main.css
:linkcss:
:docinfo1:
:icons: font
...
.`POST /api/subject/problem/render` - render an exercise [[POST_api_subject_problem_render]]
[%collapsible]
====
...
The User can access it via the <<GET_api_subject_static_.subject_.filename,`GET /api/subject/static/:subject/:filename`>> request.

====

.`POST /api/subject/problem/submit` - submit an answer & store it [[POST_api_subject_problem_submit]]
[%collapsible]
====
...
.`POST /api/subject/static/list` - list static files [[POST_api_subject_static_list]]
[%collapsible]
====
...
The User can upload it via the <<PUT_api_subject_static_.subject_.filename,`PUT /api/subject/static/:subject/:filename`>> request.
...
* This request doesn't authenticate, unlike a <<POST_api_auth_login,`POST /api/auth/login`>> one.
// login+:email!root
...
.`POST /api/session/reset` - drop and create a session [[POST_api_session_reset]]
[%collapsible]
====

NOTE: Remember to save a report via the <<POST_api_session_report_save,`POST /api/session/report/save`>> request.
...
// irreversible
* It is https://www.diki.pl/slownik-angielskiego?q=irreversible[irreversible].
++++
...
.`POST /api/session/add` - add an exercise to the session [[POST_api_session_add]]
[%collapsible]
====
...
.`POST /api/session/delete` - remove the exercise from the session [[POST_api_session_delete]]
[%collapsible]
====
...
.`POST /api/session/list` - generate a list of exercises in the current session for the users [[POST_api_session_list]]
[%collapsible]
====
...
// done-:def
* The `done` property can be `null` or a number from 0 up to and including 1, see https://github.com/Pikne-Programy/pikne-zadania/issues/24#issuecomment-782939873[#24].
// subject&exerciseId-:links2problem
* The `subject` and the `exerciseId` properties mention exercises added to the session.
The User can access it via the <<POST_api_subject_problem_render,`POST /api/subject/problem/render`>> request.

====

.`POST /api/session/status` - generate a current report without saving it [[POST_api_session_status]]
[%collapsible]
====

// done-in-exercises:def [null, 0, 0.3333333333333333 , 1]
...
.`POST /api/session/end` - end a session / block adding new submits [[POST_api_session_end]]
[%collapsible]
====
...
.`POST /api/session/report/list` - list reports [[POST_api_session_report_list]]
[%collapsible]
====
...
// generated-reports
* Generated reports can be downloaded via the <<GET_api_session_static_.filename,`GET /api/session/static/:filename`>> request.


====

.`POST /api/session/report/save` - save a report to disk [[POST_api_session_report_save]]
[%collapsible]
====
...
.`GET /api/session/static/:filename` - get a static file [[GET_api_session_static_.filename]]
[%collapsible]
====
...
.`POST /api/session/report/delete` - delete a report [[POST_api_session_report_delete]]
[%collapsible]
====
...
// done-in-exercises:def
* The `exercises` properties nested in `report` consist of elements like `null` or a number from 0 up to and including 1, see https://github.com/Pikne-Programy/pikne-zadania/issues/24#issuecomment-782939873[#24].
