...
- restructure and add better request of static files
...
- The `problem` property can contain links to static content. The User can access it via the `GET /static/:subject/:file` request. <!--problem-:links2static-->
...
- There can be an optional `done` property if the User is authorized and `raw` is `false`. <!--done-?:raw=false-->
- The `done` property can be `null` or a number from 0 up to and including 1, see [#24](https://github.com/Pikne-Programy/pikne-zadania/issues/24#issuecomment-782939873). <!--done-:def-->
...
- The `content` property can contain links to static content. The User can upload it via the `<!-- TODO -->` request. <!--content+:links2static-->
...
- This request doesn't authenticate, unlike a `POST /api/auth/login` one. <!--/register:not-authenticate-->
